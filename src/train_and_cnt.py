...
pass_rate_file = save_weights_path + 'pass_rate.json'
...
def Validate(iter, sample=0):
    ...
    if sample == 1:
        ...
        for path in validate_path:
            ...
            # show some examples
        pass_rate = sum([1 for k in psnrs if k > 20]) / r # 合格率
        pass_rates = {}
        if os.path.exists(pass_rate_file):
            with open(pass_rate_file, 'r') as f:
                pass_rates = json.load(f)
        pass_rates[iter] = pass_rate
        with open(pass_rate_file, 'w') as f:
            json.dump(pass_rates, f)
...
if os.path.exists(pass_rate_file):    # 需要先清空，重新记录
        os.remove(pass_rate_file) 
